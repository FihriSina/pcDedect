    ...
    try:
        for device in c.Win32_PnPEntity():
            ...
            is_bluetooth = (
                ...
                or "bluetooth" in name.lower()
                or "bluetooth" in manufacturer.lower()
            )

            if is_bluetooth:
                if pnp_class == "bluetooth" and name.lower() in {"bluetooth", "bluetooth device"}:
                    continue
                bluetooth_items.append(name)
                continue
            ...
            if pnp_class in useful_classes:
                ...
                if pnp_class == "usb":
                    ...
                    if not any(keyword in lowered_name for keyword in [
                        "logitech", "razer", "steelseries", "hyperx", "corsair",
                        "asus", "lenovo", "hp", "dell", "acer", "msi",
                        "webcam", "camera", "headset", "keyboard", "mouse",
                        "receiver", "wireless", "audio", "speaker", "microphone"
                    ]):
                        continue
